...
async def create_item(item: ItemCreate, db: aiosqlite.Connection = Depends(get_db)):
...
async def read_items(db: aiosqlite.Connection = Depends(get_db)):
    items = await list_items(db)
    return items


@router.put("/items/{item_id}", dependencies=[Depends(get_current_user)])
async def update_item(item_id: int, item: ItemCreate, db: aiosqlite.Connection = Depends(get_db)):
...
async def delete_item(item_id: int, db: aiosqlite.Connection = Depends(get_db)):
